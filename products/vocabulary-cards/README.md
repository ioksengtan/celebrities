...
專案根目錄已經有 `.claude/launch.json`，用 Claude Code 打開時可以直接用內建瀏覽器預覽（會用 `python -m http.server` 起一個本機伺服器）。卡片資料由 JSON 載入，因此必須透過本機 HTTP 伺服器或 GitHub Pages 開啟，不能直接使用 `file://`。

加下一個月的每日挑戰：編輯 [`daily_schedule.json`](daily_schedule.json)，每個日期寫恰好 3 個不重複、且存在於 `cards_data.json` 的 id，存檔後跑 `npm run validate`。

## 待辦 / 可擴充方向

- [ ] 持續從資料庫的 30 場演講擴充卡片與語錄，並在 `daily_schedule.json` 排進每日挑戰
