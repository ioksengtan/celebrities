...
## 2026-09-24（每日挑戰排程延長到 11 月中）

- 原排程只到 2026-10-12；因為 09-18 改版後排程用完**不會**回退成日期種子隨機抽（會顯示「尚未公布」），10-13 起會直接斷開，所以先延長，不等快到期才補
- 兩產品的 `daily_schedule.json` 都補到 2026-11-15（新增 34 天：名人詞彙每天 5 張、金句每天 3 張）
- 產生方式：卡池洗牌後照組距發，一輪洗完（名人詞彙 145 張／5＝29 天、金句 45 張／3＝15 天）才重洗，保證同一張字卡在一輪內不會再出現；只新增日期，沒動 09-15～10-12 既有排程
- `npm run validate` 通過
- 待辦：11 月中前要再延；之後可以考慮排程自動化或拉長一次排更久，減少手動補的頻率

## 2026-09-18（新手上路產品名：中文為主、英文為輔）

- Yu-Sheng 決定給新手上路用中文產品名，英文名單獨不夠直覺
- **金句**＝Keynote Lexicon；**名人詞彙**＝Executive English（不是「名人詞庫」）
- 首頁路徑卡、站內導覽、兩個產品頁標題／`<title>`／分享標題改成中文主標、英文小字副標；檔名、路徑、URL、localStorage key、資料夾名稱都不改，舊連結繼續有效

## 2026-09-18（每日挑戰改為編輯排程，不再用日期種子）

- 金句（每天 3 張）與名人詞彙（每天 5 張）的每日挑戰改由產品內的 `daily_schedule.json` 指定：日期 → 卡片 id 清單。打開當天就顯示該日排程，不再用 `hash`／`mulberry32` 從卡池抽
- **日曆**：已排程且日期已到（含今天、過去）可點開；未來已排程日顯示「尚未開放」、不能提前開包。沒排到的日子顯示「尚未公布」，不回退成種子洗牌。過去若已有完成／開卡紀錄仍可回看
- **固定性**：過去某日的牌就是 JSON 裡那組 id；之後卡池變大變小也不會重抽那天。id 若已不在 `CARDS`，略過缺卡、不整日重抽
- **金句封存**：排程是編輯決定的，產生時不再過濾封存。當天已排進挑戰的卡即使中途封存，當日牌組仍留著讓你開完；封存只影響滑卡／整理瀏覽。之後若不想讓某張再出現，請改行程而不是靠封存來抽掉
- **同步代碼**：牌組來自 shipped JSON，不必再傳鎖定 id。金句維持 `v: 2`（完成日 `d`、今日已開卡 `o`、封存 `a` 聯集合併）；名人詞彙維持 `v: 1`（`d`＋`o`）。舊代碼仍可匯入
- **編輯怎麼加下個月**：在各產品的 `daily_schedule.json` 加上 `"YYYY-MM-DD": [id, …]`，每天恰好 3 或 5 個不重複且存在的 id；可先排約一個月。改完跑 `npm run validate`（張數不對、id 不存在、同一天重複會失敗）
- 目前已排 2026-09-15～2026-10-12（四週），Pages 可直接玩
...
## 2026-09-18（日曆可查看所有歷史挑戰）

- 修正 Keynote Lexicon 與 Executive English 日曆：所有今天以前的日期都可點擊，不再只限今天或已有完成／開卡紀錄的日期
- 過去未完成日期會依日期種子重建固定牌組，並以已揭示的唯讀狀態顯示；查看歷史不會寫入完成紀錄、不會回補連續天數
- 未來日期維持不可點擊
...
## 2026-09-19（Nadella 訪談入庫）

- 新增 SN-UNDATED-001、Satya Nadella 講者資料、完整使用者逐字稿、繁體中文導讀與摘要，以及 5 則附段落時間碼的 draft 語錄。
- 同步主資料庫總數 31、主題／類型／標籤／長度／講者索引；時間索引放在 unknown，未以匯入日冒充訪談日。
- 核心頁加入講者名稱對應；單場深連結可載入摘要，語錄待影片校聽，尚不上已核實語錄牆。未發布至遠端。
