...
## 2026-09-18（每日挑戰：當日牌組鎖定）

- 第一次打開某日挑戰時，把抽出的卡片 id 寫進本機鎖定；之後同一裝置再看同一天，即使卡池增減或 金句 封存集合變了，也重用這份清單
- 本機 key：`keynote-lexicon-daily-packs` / `exec-vocab-daily-packs`，格式 `{ "YYYY-MM-DD": [id, …] }`。完成日、已開卡 key 不變
- 首次產生仍走日期種子（金句 產生時排除當時已封存，並優先保留該日已開卡）。鎖定之後不再用封存過濾把當日牌抽掉，當天仍可開完
- 鎖定 id 若已不在現行 `CARDS`，保留其餘張，只用日期種子補缺、不整日重抽；補齊後寫回該日鎖定
- 同步代碼升為 `v: 3`，新增 `p`（各日鎖定牌組）。舊 `v: 1`／`v: 2` 仍可匯入。合併規則：完成日、今日已開卡、封存仍是聯集；某日牌組採「本機已有則保留，沒有才收下對方的」，避免一台覆蓋另一台已鎖定的牌
- 未開過的日期第一次點進日曆仍會依種子產生，隨即鎖定
