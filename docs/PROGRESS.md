...
## 2026-09-06（產品優先順序：先做帳號與跨裝置同步）

- 目前最值得優先做的下一步不是再補更多卡片，而是把「單次體驗」變成「會回來的學習系統」
- 優先順序建議：
  1. 帳號 + 跨裝置同步 + 每日挑戰/連續天數持久化（最重要）
  2. 社群分享增強（OG image、卡片分享、深連結）
  3. 個人化復習（依錯誤率與熟練度安排重複練習）
  4. 再擴充內容與講者資料
- 目前的主要阻礙是：學習進度只存在瀏覽器 `localStorage`，換裝置／清資料後會歸零，會直接削弱使用者留存與分享的價值
- 建議的 MVP：用 GitHub / Google 登入，存儲已熟練卡、每日挑戰紀錄、連續天數、學習歷史；此功能可直接把現有 HTML 原型提升成真正的產品，而不需要先做大規模內容開發
- 若要在未來與團隊共用，這一項可作為產品下一步的明確決策基準：先做「持續學習系統」，再做深度內容擴充
